"""Script to fine-tune a chemical language model."""
...
from poeclm.model import GPT
from poeclm.utils import round_up
...
class TopConfig:
    ...
    checkpoint_file: str = "outputs/pretraining/enum_16M/85M/best.ckpt"
    vocab_file: str = "data/vocab.txt"
    dataset_dir: str = "data/datasets/dockstring"
    ...
    output_dir: str = "outputs/finetuning/dockstring/85M"
...
def main() -> None:
    ...
    OmegaConf.save(cfg, output_dir / "finetuning.yaml")
    wandb.init(
        project=f"poeclm-finetuning-{dataset_dir.name}",
        config=OmegaConf.to_container(cfg),  # type: ignore
    )
    wandb.save(str(output_dir / "finetuning.yaml"), str(output_dir))
    ...
    model = GPT.from_checkpoint_file(checkpoint_file)
    ...
    print(f"Train size: {len(datamodule.get_dataset('train')):,}")

    max_iters = round_up(
        config.max_epochs
        * len(datamodule.get_dataset("train"))
